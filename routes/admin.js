...
router.get("/items", requireLogin, requireAdmin, async (req, res) => {
  const db = getDb();
  const lost = await db.collection("lost_items").find().sort({ createdAt: -1 }).toArray();
  const found = await db.collection("found_items").find().sort({ createdAt: -1 }).toArray();
 ...
 */
router.delete("/lost-items/:id", requireLogin, requireAdmin, async (req, res) => {
  let _id;
 ...
 */
router.delete("/found-items/:id", requireLogin, requireAdmin, async (req, res) => {
  let _id;
  try { _id = new ObjectId(req.params.id); }
  catch { return res.status(400).json({ error: "invalid id" }); }
